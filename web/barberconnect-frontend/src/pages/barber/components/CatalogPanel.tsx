    ...
    <div className="flex flex-col gap-8 animate-fade-in pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        {loading ? (
          ...
          <p className="text-slate-500">No haircut styles found. Click "Add Haircut" to get started.</p>
        ) : styles.map((style) => (
          <div key={style.haircut_style_id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden group hover:shadow-md transition-shadow flex flex-col h-full">
            {/* Image */}
            ...
            <div className="p-6 flex flex-col flex-1">
              <h3 className="text-lg font-bold text-slate-800 mb-1">{style.name}</h3>
              <p className="text-sm text-slate-500 font-medium mb-4 flex-1">{style.description}</p>
              
              <div className="flex items-center gap-4 mb-3">
                <span className="font-extrabold text-slate-800 tracking-tight flex items-center gap-1">
                  <span className="text-slate-400 text-sm font-normal">₱</span> {style.basePrice}
                </span>
                <span className="text-slate-400 font-bold flex items-center gap-1 text-sm">
                  <span className="text-sm">🕒</span> {style.durationMinutes} min
                </span>
              </div>
              
              <p className="text-xs text-slate-400 font-semibold mb-5">{style.isActive ? 'Active' : 'Inactive'}</p>
